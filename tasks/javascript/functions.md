...
```js
describePerson.call({ name: "Bob" }, "Bob", 30); // "Bob is 30 years old."
```
...
```js
executeFunction(() => console.log("Executed!")); // "Executed!"
```
...
```js
const add1 = x => x + 1; const double = x => x \* 2; const addThenDouble = compose(double, add1); console.log(addThenDouble(3)); // 8
```
...
```js
showContext.call({ name: "Context" }); // Logs the context object
```
...
```js
logAllArguments(1, 2, 3, "hello"); // Logs: 1, 2, 3, "hello"
```
...
```js
borrowMethod.call(objA, objB.method); // Executes method from objB in context of objA
```
...
```js
const result = chain(obj).method1().method2(); // Chained methods executed
```
...
```js
console.log(average.apply(null, [1, 2, 3, 4])); // 2.5
```
...
console.log(randomInRange()); // Random number between 1 and 10
```
...
```js
const countedFunc = countCalls(someFunc);
countedFunc(); // 1
```
...
```js
invokeWithDifferentContext.call(obj, methodName); // Invokes method with obj context
```
...
```js
const cachedFunc = cacheResults(expensiveFunction);
console.log(cachedFunc(arg)); // Cached result
```
...
```js
console.log(maxOf.apply(null, [1, 2, 3, 4])); // 4
```
